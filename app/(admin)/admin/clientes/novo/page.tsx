...
export default async function NovoClientePage() {
  ...
  const { data: owners } = await supabase.from('profiles').select('id, full_name, email').eq('role', 'client_owner').order('full_name');

  return (
    <div>
      <TopHeading title="Novo cliente" description="Crie um novo negócio dentro da plataforma." />
      <form action={createBusiness} className="rounded-[1.5rem] border border-border bg-white p-6 shadow-sm">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            <Field label="Endereço"><Input name="address" /></Field>
          </div>
          <div className="md:col-span-2">
            <Field label="Descrição"><Textarea name="description" rows={4} /></Field>
          </div>
        </div>
        <div className="mt-6"><SubmitButton>Criar negócio</SubmitButton></div>
      </form>
    </div>
  );
}
