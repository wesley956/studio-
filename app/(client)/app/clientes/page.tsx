...
export default async function ClientesPage() {
  ...
  const { data: customers } = await supabase.from('customers').select('*').eq('business_id', business?.id).order('created_at', { ascending: false });
  ...
  return (
    <div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
        ...
        <form action={createCustomer} className="rounded-[1.5rem] border border-border bg-white p-6 shadow-sm">
